...
public class ChatRoomService {
     ...
     * 채팅방 생성 또는 기존 채팅방 반환
     *
     * @param requestDto 채팅방 생성 요청 데이터 (상대방 ID만 포함)
     * @param user 세션에서 주입된 현재 로그인 사용자 정보
     * @return 생성되거나 기존의 채팅방 정보
     *
     * 처리 과정:
     * 1. 입력 검증: 세션 사용자와 상대방 멤버 존재 여부 확인
     * 2. 강아지 검증: dogId가 있다면 해당 강아지 존재 여부 및 소유자 확인
     * 3. 중복 체크: 동일한 두 멤버 간 기존 채팅방 존재 여부 확인
     * 4. 채팅방 처리: 기존 채팅방이 있으면 반환, 없으면 새로 생성 및 멤버 추가
     */
    @Transactional
    public ChatRoomResponseDto createChatRoom(ChatRoomCreateRequestDto requestDto, SessionUser user) {
        // 1. 세션 사용자(현재 로그인 사용자) 검증
        Member currentMember = chatRoomValidator.validateAndGetMember(user.getMemberId());

        // 2. 상대방 멤버 검증
        Member otherMember = chatRoomValidator.validateAndGetMember(requestDto.getOtherMemberId());

        // 3. 자신과 채팅방을 만들려고 하는 경우 방지
        if (currentMember.getMemberId().equals(otherMember.getMemberId())) {
            throw new ChatException(ErrorCode.CHAT_INVALID_SAME_MEMBER);
        }

        // 4. 강아지 검증 (존재 여부 + 소유자 검증)
        Dog dog = chatRoomValidator.validateAndGetDog(requestDto.getDogId(), currentMember, otherMember);

        // 5. 중복 채팅방 체크
        Optional<ChatRoom> existingChatRoom = duplicateChecker.findExistingChatRoom(
                dog, currentMember.getMemberId(), otherMember.getMemberId()
        );

        if (existingChatRoom.isPresent()) {
            // 기존 채팅방이 있다면 두 멤버를 모두 재활성화하고 반환
            ChatRoom chatRoom = existingChatRoom.get();
            reactivateMembersIfNeeded(chatRoom, currentMember, otherMember);
        ...
        // 8. 채팅방 멤버 추가 (세션 사용자와 상대방)
        createAndAddChatRoomMember(savedChatRoom, currentMember);
        createAndAddChatRoomMember(savedChatRoom, otherMember);
